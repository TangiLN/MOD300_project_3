...
class TestSimulationBox: 
    """Class to test the Simulation_box """
    def __init__(self):
        """
        Function to create a TestSimualtionBox
        """
        self.box=simulation_box(0,20,0,20,0,20)
    def test_walker_simulation_no_sphere(self,steps):
        """ 
        Unit Test for the walker method, here we test on a simulation with no sphere, 
        wich mean that the estimated volume of sphere should be around 0.
        The result could be slighly over 0 if the walkers are not going on all the map, 
        that's why we add abs_error of 200m^3
        """
    ...
    def test_walker_simulation_with_sphere(self,steps):
        """ 
        Unit Test for the walker method, here we test on a simulation with many spheres,
        wich mean that the estimated volume of sphere should be around the volume of the box.
        The result could be slighly less then 8000 because the distribution of sphere is random, 
        that's why we add rel_error of 5% of the volume of the box.
        """
        print("\n--- Start of test with many spheres---\n")
        self.box = simulation_box(0,20,0,20,0,20)
        self.box.generate_sphere(n=300,specific=False)
        self.box.generate_starting_walker(n=100)
        Monte_Carlo_volume=self.box.start_walker_simulation(steps=steps)
    ...
    def test_walker_simulation_random(self,steps,n_s=5):
        ...
        We calculate the result with walker method and 
        we compare it to the analytical calculation from the data of the sphere. 
        The result might be different with big values of sphere 
        because the analytical calculation add each volume of sphere, 
...
def launch_test():
    """
    Function to launch the test for the part 1
    I commented the 'test_part_1_task_10_volume' because the function is not stable
    """
